...
function getCjBase(env) {
  // Uses the Render proxy if configured (to dodge Cloudflare's shared-IP block),
  // otherwise falls back to calling CJ directly.
  return env.CJ_PROXY_BASE || "https://developers.cjdropshipping.com/api2.0/v1";
}
...
async function getValidToken(env) {
  ...
  if (cached && cached.refreshToken && new Date(cached.refreshTokenExpiryDate) > new Date()) {
    const refreshed = await fetch(`${getCjBase(env)}/authentication/refreshAccessToken`, {
      method: "POST",
  ...
  // Fresh token
  const fresh = await fetch(`${getCjBase(env)}/authentication/getAccessToken`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
async function cjFetch(env, path, options = {}) {
  ...
  const res = await fetch(`${getCjBase(env)}${path}`, {
    ...options,
    headers: {
...
// ---------- Smart Pricing: margin scales with product cost ----------
function applyMarkup(costPrice, env) {
  const cost = parseFloat(costPrice);
  if (!cost || cost <= 0) return 0;

  // Tiered margin: cheaper items get a higher % (to stay competitive),
  // pricier items get a lower % but a much higher absolute profit.
  let pct;
  if (cost < 10) pct = 55;
  else if (cost < 25) pct = 45;
  else if (cost < 60) pct = 35;
  else if (cost < 150) pct = 25;
  else pct = 18;

  let price = cost * (1 + pct / 100);

  // Absolute profit floor so low-cost items still net a minimum $ profit,
  // and a small extra bump on high-ticket items.
  const minProfit = cost < 25 ? 4 : cost < 60 ? 8 : cost < 150 ? 15 : 30;
  if (price - cost < minProfit) price = cost + minProfit;
...
async function handleCategories(req, env) {
  const data = await cjFetch(env, `/product/getCategory`, { method: "GET" });
  if (!data.result) {
    return new Response(JSON.stringify({ error: data.message }), {
      status: 500,
      headers: { "Content-Type": "application/json", ...corsHeaders(env) },
  ...
  // CJ returns a nested category tree; flatten to top-level + children for a simple filter bar.
  const categories = (data.data || []).map((first) => ({
    id: first.categoryFirstId,
    name: first.categoryFirstName,
    children: (first.categoryFirstList || []).map((second) => ({
      id: second.categorySecondId,
...
// ---------- Notion: log order automatically ----------
async function pushOrderToNotion(env, orderRecord, cjResult) {
  if (!env.NOTION_TOKEN || !env.NOTION_DATABASE_ID) return; // optional integration
  try {
    await fetch("https://api.notion.com/v1/pages", {
      ...
      body: JSON.stringify({
        ...
        properties: {
          "Order Number": { title: [{ text: { content: orderRecord.orderNumber } }] },
          "Customer Name": { rich_text: [{ text: { content: orderRecord.shippingCustomerName || "" } }] },
          Email: { email: orderRecord.email || null },
          Phone: { phone_number: orderRecord.shippingPhone || null },
          City: { rich_text: [{ text: { content: orderRecord.shippingCity || "" } }] },
          Country: { rich_text: [{ text: { content: orderRecord.shippingCountryCode || "" } }] },
          "Total (USD)": {
            number: (orderRecord.products || []).reduce(
              (s, p) => s + (p.sellingPrice || 0) * (p.quantity || 1), 0
            ),
          },
          Status: { select: { name: cjResult.result ? "Fulfilled" : "Fulfillment Failed" } },
          "CJ Order ID": { rich_text: [{ text: { content: String(cjResult.data?.orderId || "") } }] },
        },
      }),
    });
  } catch (err) {
    console.error("Notion sync failed:", err.message); // never block order fulfillment on this
  }
}
...
async function handlePaymentWebhook(req, env) {
  ...
  await env.TOKEN_STORE.put(`order:${orderNumber}`, JSON.stringify(stored));
  await pushOrderToNotion(env, stored, cjResult);
...
// ---------- Newsletter signup ----------
async function handleNewsletterSignup(req, env) {
  const body = await req.json();
  ...
  if (env.NOTION_TOKEN && env.NEWSLETTER_DATABASE_ID) {
    try {
      await fetch("https://api.notion.com/v1/pages", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          parent: { database_id: env.NEWSLETTER_DATABASE_ID },
          properties: {
      ...
      console.error("Newsletter Notion sync failed:", err.message);
    }
  }
...
export default {
  async fetch(req, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/categories") return await handleCategories(req, env);
      ...
      if (url.pathname === "/api/newsletter" && req.method === "POST") return await handleNewsletterSignup(req, env);
